...
async function bootstrap(): Promise<AgentRunner> {
  ...
  try {
    ...
    if (!apiKey) {
      throw new Error(
        "GROQ_API_KEY not found in environment variables. Get one at: https://console.groq.com",
      );
    }

    const { GroqLLMService } = await import("../server/llm/llm-service");
    const llmService = new GroqLLMService(apiKey);
    console.log("  ✅ Groq LLM Service connected (Llama 3.3 70B)");
